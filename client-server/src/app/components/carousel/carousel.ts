...
export class Carousel {
  ...
  constructor(properties: Properties) {
    ...
    this.visibleWidth =
      this.properties.visibleWidth || this.element.parentElement.clientWidth;
  ...
  handleTouchstart = (event: any) => {
    ...
    if (touches) {
      this.startX =
        touches[0].clientX - this.getCarouselElementPosition()['left'];
    } else {
      this.startX = event.clientX - this.getCarouselElementPosition()['left'];
    }
    this.containerInitialPositionX =
      this.getElementPosition()['left'] -
      this.getCarouselElementPosition()['left'];
  ...
  getContainerPosition() {
    ...
    return (
      this.initialContainerPosition +
      this.newContainerPositionIndex * this.fullCellWidth +
      correction
    );
  }
  ...
  getContainerPositionCorrection() {
    ...
    if (
      this.images.length - this.preliminarySlideCounter <
        this.visibleCellsCount ||
      this.isSlideLengthLimited
    ) {
      if (
        this.visibleWidth <
        this.totalContainerCellsCount * this.fullCellWidth
      ) {
        correction = -(
  ...
  getContainerWidth() {
    this.totalContainerCellsCount =
      this.visibleCellsCount + this.overflowCellsLimit * 2;
  ...
  handleSlide(slideLength: number = undefined): void {
    ...
    this.slideLength = slideLength
      ? slideLength
      : this.limitSlideLength(this.slideLength);
    ...
    if (this.direction === 'left' && !this.isSlideInProgress) {
      ...
      if (!this.detectLastSlide(this.slideCounter + this.slideLength)) {
        this.newContainerPositionIndex =
          this.newContainerPositionIndex - this.slideLength;
        this.isSlideInProgress = true;

        if (this.isLazyLoad) {
          this.isContainerLocked =
            this.preliminarySlideCounter > this.overflowCellsLimit;
  ...
  getSlideLength() {
    ...
    let length = Math.floor(
      (this.distanceAbs + correction) / this.fullCellWidth
    );
  ...
  isNextArrowDisabled() {
    if (this.visibleCellsOverflowContainer) {
      return (
        this.detectLastSlide(this.slideCounter + 1) &&
        this.isContainerPositionCorrection
      );
    } else {
  ...
  detectContainerUnlock() {
    return (
      this.images.length - this.preliminarySlideCounter <
      this.visibleCellsCount + this.overflowCellsLimit
    );
  }
  ...
  get containerOverflowRightCount() {
    let totalOverflowCellsCount =
      this.images.length - this.totalContainerCellsCount;
    let overflowRight = 0;

    if (totalOverflowCellsCount > 0) {
      overflowRight =
        totalOverflowCellsCount -
        (this.previousSlideCounter - this.overflowCellsLimit);
